...
function CUST_customerTermcalenderdeletion(custid,calenderIDcode,startdate,start_time_in,start_time_out,enddate,end_time_in,end_time_out,formname)
{  
  var cal = CalendarApp.getCalendarsByName(calenderIDcode)[0] ; 
  ...
  for(var q=0;q<startevent_deletion.length;q++)
  {
    if(startevent_deletion[q])
    {
      ...
      if(parseInt(start_desc[0]) == custid&&ev_title.match("CHECKIN"))
      {
  ...
  for(var q=0;q<endevent_deletion.length;q++)
  {
    if(endevent_deletion[q])
    {
      ...
      if(parseInt(end_desc[0]) == custid&&ev_title.match("CHECKOUT"))
      {
...
function StarHubUnit_DeleteCalEvent(unitno,calenderIDcode,startdate,start_time_in,start_time_out,enddate,end_time_in,end_time_out,formname)
{
  ...
  for(var q=0;q<startevent_deletion.length;q++)
  {
    if(startevent_deletion[q])
    {
      var start_desc=startevent_deletion[q].getDescription();
      start_desc=start_desc.split(' - ');
      if(start_desc[1]!=undefined&&(start_desc[1].toString().match("EI")&&formname=="UNIT")||(formname=="INTERNET")||(formname=="CABLE"))
      {
        if((start_desc[0]==unitno&&formname=="UNIT")||(start_desc[0]==unitno && start_desc[2].toString().match(formname)))
        {
          startevent_deletion[q].deleteEvent(); // Deleting events if any edit occure
        }
      }
  ...
  for(var q=0;q<endevent_deletion.length;q++)
  {
    if(endevent_deletion[q])
    {
      var end_desc=endevent_deletion[q].getDescription();
      end_desc=end_desc.split(' - ');
      if(end_desc[1]!=undefined && end_desc[1].toString().match("EI")&&formname=="UNIT"||formname=="INTERNET" ||formname=="CABLE")
      {
        if((end_desc[0]==unitno &&formname=="UNIT")||(end_desc[0]==unitno && end_desc[2].toString().match(formname)))
        {
          endevent_deletion[q].deleteEvent(); // Deleting events if any edit occure
        }
      }
...
function CTermExtn_Calevent(conn,CTermExtn_custid,CTermExtn_recver,calenderIDcode,ctermformname,successflag)
{
  ...
  CTermExtn_tempstmt.execute("CALL SP_CUSTOMER_MIN_MAX_RV("+CTermExtn_custid+",@MIN_LP,@MAX_LP)");
  ...
  {
    cterm_minlp=CTermExtn_temptblres.getString("@MIN_LP");
  }
  ...
  while(CTermExtn_calevntres.next())
  {
    ...
    if(CTermExtn_gstcard==null)
    {
      ...
      CUST_customerTermcalenderdeletion(CTermExtn_custid,calenderIDcode,CTermExtn_stdate,CTermExtn_start_time_in,CTermExtn_start_time_out,CTermExtn_eddate,CTermExtn_end_time_in,CTermExtn_end_time_out,"")
...
function CTermExtn_GetCalevent(conn,CTermExtn_custid)
{
  var CTermExtn_custfirstname="",CTermExtn_custlastname="";
  var CTermExtn_calevntchk_flag=0;
  var CTermExtn_prevunitno=[];
  ...
  while(CTermExtn_calevntres.next())
  {
    ...
    if(CTermExtn_gstcard==null)
    {
      ...
      var CTERM_final_CalEvents={'sddate':CTermExtn_stdate,'sdtimein':CTermExtn_start_time_in,'sdtimeout':CTermExtn_start_time_out,'eddate':CTermExtn_eddate,'edtimein':CTermExtn_end_time_in,'edtimeout':CTermExtn_end_time_out};
      CTERM_finalEvents.push(CTERM_final_CalEvents)  
    }
  }
  return CTERM_finalEvents;
}
...
function CUST_CREATION_customercalenderdeletion(customer_id,calenderIDcode,calevent_array)
{
  ...
  for(var c=0;c<calevent_array.length;c++)
  {
    ...
    var startevents=CalenderTime_Convertion(calenderIDcode,eventdetails[0], eventdetails[1], eventdetails[2]);
    var startevent_deletion=cal.getEvents(startevents[0], startevents[1]);//getting start date event from calendar
    for(var q=0;q<startevent_deletion.length;q++)
    {
      if(startevent_deletion[q])
