...
{ 
  ...
  function CTERM_getErrMsgCalTime()
  {
    ...
    CTERM_Dropuntermcustsp(CTERM_errcon,CTERM_temptable[0]);
    CTERM_Dropuntermcustsp(CTERM_errcon,CTERM_temptable[1]);
    CTERM_Dropuntermcustsp(CTERM_errcon,CTERM_temptable[2]);
    CTERM_Dropuntermcustsp(CTERM_errcon,CTERM_temptable[3]);
    CTERM_errcon.commit();
    CTERM_errcon.close();
  ...
  function CTERM_getCustomerName(CTERM_form)
  {
    ...
    {
      CTERM_Dropuntermcustsp(CTERM_cnamecon,CTERM_temptable[0]);
      CTERM_Dropuntermcustsp(CTERM_cnamecon,CTERM_temptable[1]);
      CTERM_Dropuntermcustsp(CTERM_cnamecon,CTERM_temptable[2]);
      CTERM_Dropuntermcustsp(CTERM_cnamecon,CTERM_temptable[3]);
    }
  ...
  function CTERM_getCustomerId(CTERM_form)
  {
    ...
    if(CTERM_form_option=='CTERM_radio_untermnonactive')
    {
      CTERM_Dropuntermcustsp(CTERM_cidcon,CTERM_temptable[0]);
      CTERM_Dropuntermcustsp(CTERM_cidcon,CTERM_temptable[1]);
      CTERM_Dropuntermcustsp(CTERM_cidcon,CTERM_temptable[2]);
      CTERM_Dropuntermcustsp(CTERM_cidcon,CTERM_temptable[3]);
    }
    CTERM_cidcon.close();
  ...
  function CTERM_getCustomerdtls(CTERM_custid,CTERM_radio_termoption)
  {
    ...
    {     
      var CTERM_temptable=CTERM_Calluntermcustsp(CTERM_custdtlsconn);
      var CTERM_custdtlsquery="SELECT CED.UASD_ID,CACD.CACD_VALID_TILL,CED.CED_LEASE_PERIOD,CED.CED_QUARTERS,CED.CED_RECHECKIN,CED.CED_PRETERMINATE,CTD.CLP_TERMINATE,U.UNIT_NO,UASD.UASD_ACCESS_CARD,CTD.CLP_GUEST_CARD,C.CUSTOMER_FIRST_NAME,C.CUSTOMER_LAST_NAME,CTD.CLP_STARTDATE,CTD.CLP_ENDDATE,NC.NC_DATA,TF.CC_DEPOSIT,TF.CC_PAYMENT_AMOUNT,CCD.CCD_COMPANY_NAME,CCD.CCD_COMPANY_ADDR,CCD.CCD_POSTAL_CODE,TF.CC_ELECTRICITY_CAP,TF.CC_AIRCON_FIXED_FEE,TF.CC_AIRCON_QUARTERLY_FEE,TF.CC_DRYCLEAN_FEE,TF.CC_PROCESSING_FEE,TF.CC_CHECKOUT_CLEANING_FEE,CPD.CPD_EP_NO,CPD.CPD_EP_DATE,CPD.CPD_PASSPORT_NO,CPD.CPD_PASSPORT_DATE,CED.CED_NOTICE_PERIOD,CED.CED_NOTICE_START_DATE,CPD.CPD_DOB,CPD.CPD_MOBILE,CPD.CPD_INTL_MOBILE,CPD.CPD_EMAIL,CCD.CCD_OFFICE_NO,CED.CED_EXTENSION,CED.CED_REC_VER,CED.CED_CANCEL_DATE,CPD.CPD_COMMENTS,ULD.ULD_LOGINID,CTD.CLP_PRETERMINATE_DATE,DATE_FORMAT(CONVERT_TZ(CTD.CLP_TIMESTAMP,"+timeZoneFormat+"),'%d-%m-%Y %T') AS CTDTIMESTAMP FROM  CUSTOMER_ENTRY_DETAILS CED  LEFT JOIN CUSTOMER_LP_DETAILS CTD ON CED.CUSTOMER_ID=CTD.CUSTOMER_ID JOIN  "+CTERM_temptable[0]+" TFTDC ON CTD.CUSTOMER_ID=TFTDC.TFTDC_CUSTOMERID AND TFTDC.TFTDC_UASDID=CTD.UASD_ID AND TFTDC.TFTDC_RECVER=CTD.CED_REC_VER AND(CTD.CED_REC_VER=CED.CED_REC_VER)LEFT JOIN CUSTOMER_COMPANY_DETAILS CCD ON CED.CUSTOMER_ID=CCD.CUSTOMER_ID LEFT JOIN CUSTOMER_ACCESS_CARD_DETAILS CACD ON CED.CUSTOMER_ID=CACD.CUSTOMER_ID AND CACD.CACD_VALID_TILL IS NULL LEFT JOIN "+CTERM_tempfeetblname+" TF ON  CED.CUSTOMER_ID=TF.CUSTOMER_ID LEFT JOIN CUSTOMER C ON CED.CUSTOMER_ID=C.CUSTOMER_ID LEFT JOIN  CUSTOMER_PERSONAL_DETAILS CPD ON CED.CUSTOMER_ID =CPD.CUSTOMER_ID ,NATIONALITY_CONFIGURATION NC ,UNIT U,UNIT_ACCESS_STAMP_DETAILS UASD,USER_LOGIN_DETAILS ULD  WHERE   ULD.ULD_ID=CTD.ULD_ID AND  (CED.UNIT_ID=U.UNIT_ID)AND  CACD.UASD_ID=UASD.UASD_ID and CTD.UASD_ID=CACD.UASD_ID AND (CED.CUSTOMER_ID="+CTERM_custid+") AND (CPD.NC_ID=NC.NC_ID) AND (CED.CED_REC_VER=TF.CUSTOMER_VER) AND (CLP_STARTDATE<=CURDATE() OR CLP_STARTDATE>=CURDATE()) AND CACD.CACD_VALID_TILL IS NULL AND  CTD.CLP_TERMINATE IS NULL AND CED.CED_CANCEL_DATE IS NULL AND IF(CTD.CLP_PRETERMINATE_DATE IS NOT NULL,CLP_PRETERMINATE_DATE<=CURDATE(),CLP_ENDDATE<=CURDATE()) ORDER BY CED.CED_REC_VER,CTD.CLP_GUEST_CARD ASC"
    }
    ...
    var CTERM_temp_stmt= CTERM_custdtlsconn.createStatement();
    var CTERM_delete_temp = "DROP TABLE "+CTERM_tempfeetblname+"";
    CTERM_temp_stmt.execute(CTERM_delete_temp);
    CTERM_temp_stmt.close();    
    if(CTERM_radio_termoption=='CTERM_radio_untermnonactive')
    {
      CTERM_Dropuntermcustsp(CTERM_custdtlsconn,CTERM_temptable[0]);
      CTERM_Dropuntermcustsp(CTERM_custdtlsconn,CTERM_temptable[1]);
      CTERM_Dropuntermcustsp(CTERM_custdtlsconn,CTERM_temptable[2]);
      CTERM_Dropuntermcustsp(CTERM_custdtlsconn,CTERM_temptable[3]);
    }
  ...
  function CTERM_UpdatePtd(CTERM_form,CTERM_result_array)
  {
    ...
    else if(CTERM_radio_termoption=="CTERM_radio_activecust")
    {
      ...
      if(CTERM_customerptd!="")
      {
        eilib.CTermExtn_Calevent(CTERM_custdtlsconn,CTERM_custid,CTERM_recver,calenderIDcode,CTERM_customerptd,CTERM_ptdcalstime,CTERM_ptdcaletime,CTERM_calptd,"TERMINATION");
      }
    ...
    {
      var CTERM_updateflag=CTERM_updateflag_rs.getString(1);
    }
  ...
  function CTERM_Calluntermcustsp(CTERM_callspcon)
  {
    ...
    var CTERM_temptblname1="";
    var CTERM_temptblname2="";
    var CTERM_temptblname3="";
    var CTERM_temptblname4="";
    while(CTERM_feetemptblres.next())
    {
  ...
  // FUNCTION TO DROP UNTERM CUST TEMP TABLE
  function CTERM_Dropuntermcustsp(CTERM_droptablecon,CTERM_temptable)
  {
    var CTERM_droptemp_stmt= CTERM_droptablecon.createStatement();
    var CTERM_drop_tempquery = "DROP TABLE IF EXISTS "+CTERM_temptable+"";
    CTERM_droptemp_stmt.execute(CTERM_drop_tempquery);
    CTERM_droptemp_stmt.close();
  }
}
catch(err)
{
