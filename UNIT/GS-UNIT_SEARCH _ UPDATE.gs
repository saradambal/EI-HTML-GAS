...
//*******************************************UNIT SEARCH/UPDATE*********************************************//
//<!--DONE BY:PUNI
//VER 1.4 -SD:22/12/2014,ED:22/12/2014,TRACKER NO:840,added drop table function from eilib to avoid temp table drop issue for pf
...
try{
  ...
  function USRC_Data_AlreadyExists(USRC_inventory_unitno,USRC_typeofcard,USRC_flag_card_unitno,USRC_parent_func)
  {
    ...
    if((USRC_flag_card_unitno==5)||(USRC_flag_card_unitno==8)||(USRC_flag_card_unitno=='USRC_flag_check_accesscard')){
      ...
      if(USRC_flag_card_unitno=='USRC_flag_check_unitno'){
        ...
        var USRC_enddate_query="CALL SP_UNIT_CHECK_TRANSACTION((SELECT UNIT_ID FROM UNIT WHERE UNIT_NO="+USRC_inventory_unitno+"),'"+UserStamp+"',@SDATE,@EDATE,@TEMP_CHKTRANS,@TEMP_CUST,@TEMP_CUSTLP)";
        USRC_stmt_cardno.execute(USRC_enddate_query);
        USRC_stmt_cardno.close();
        var USRC_stmt_cardno = USRC_conn.createStatement();
        var USRC_cardunitno_query="SELECT @SDATE,@EDATE,@TEMP_CHKTRANS,@TEMP_CUST,@TEMP_CUSTLP";
      }
      ...
      { var USRC_flag_unitno=true;
       ...
       if((USRC_flag_card_unitno=='USRC_flag_transac_check_enddate')||(USRC_flag_card_unitno=='USRC_flag_transac_check_unitno')){
         USRC_arr_custexpense.push(USRC_card_rs.getString(1))
         USRC_arr_custexpense.push(USRC_card_rs.getString(2)); 
         eilib.DropTempTable(USRC_conn,USRC_card_rs.getString(3));
         eilib.DropTempTable(USRC_conn,USRC_card_rs.getString(4));
         eilib.DropTempTable(USRC_conn,USRC_card_rs.getString(5));
       }}
  ...
  function USRC_func_update(USRC_form_values,USRC_obj_rowvalue,USRC_obj_flex)
  {
    try
    {
      ...
      var USRC_rollback_conn =eilib.db_GetConnection();
      USRC_rollback_conn.setAutoCommit(false);
      var USRC_stmt_upd= USRC_rollback_conn.createStatement();  
      ...
      {  
        ...
        var USRC_upd_sp ="CALL SP_UNIT_UPDATE("+USRC_upd_unitid_stampid+","+USRC_upd_unitno+",'"+USRC_upd_startdate_update+"','"+USRC_upd_enddate_update+"',"+USRC_upd_unitpayment+","+USRC_upd_unitdeposit+","+USRC_upd_obsolete+","+USRC_upd_nonei+",'"+USRC_upd_comments+"','"+USRC_upd_accnoid+"','"+USRC_upd_accname+"','"+USRC_upd_bankcodeid+"','"+USRC_upd_branchcode+"','"+USRC_upd_bankaddr+"','"+UserStamp+"',@FLAG_UPDATE,@TEMP_CHK_TRANS,@TEMP_CUST,@TEMP_CUSTLP)";
        USRC_stmt_upd.execute(USRC_upd_sp); USRC_stmt_upd.close();     
        var USRC_stmt_droptemp = USRC_rollback_conn.createStatement();
        var USRC_rs_droptemp = USRC_stmt_droptemp.executeQuery("SELECT @TEMP_CHK_TRANS,@TEMP_CUST,@TEMP_CUSTLP");
        while(USRC_rs_droptemp.next())
        {    
          var USRC_temp_chktrans=USRC_rs_droptemp.getString("@TEMP_CHK_TRANS");
          var USRC_temp_cust=USRC_rs_droptemp.getString("@TEMP_CUST");
          var USRC_temp_custlp=USRC_rs_droptemp.getString("@TEMP_CUSTLP");
        }
        USRC_rs_droptemp.close();USRC_stmt_droptemp.close();
      }      
      ...
      var USRC_stmt_flag = USRC_rollback_conn.createStatement();
      ...
      if(USRC_flag_flag_update==1){
        if((USRC_upd_selectoption_unit==3)||(USRC_upd_selectoption_unit==4)||(USRC_upd_selectoption_unit==6)||(USRC_upd_selectoption_unit==7)){
          var USRC_calenderIDcode=eilib.CUST_getCalenderId(USRC_rollback_conn);
          var  USRC_sh_arr= eilib.getStarHubUnitCalTime(USRC_rollback_conn);
          ...
          eilib.StarHubUnit_DeleteCalEvent(USRC_obj_rowvalue.USRC_tr_first,USRC_calenderIDcode,USRC_oldvalues_sdate,USRC_sh_arr[0],USRC_sh_arr[1],USRC_oldvalues_edate,USRC_sh_arr[0],USRC_sh_arr[1],'UNIT')
      ...
      eilib.DropTempTable(USRC_rollback_conn,USRC_temp_chktrans);
      eilib.DropTempTable(USRC_rollback_conn,USRC_temp_cust);
      eilib.DropTempTable(USRC_rollback_conn,USRC_temp_custlp);
      USRC_rollback_conn.commit();
      USRC_rollback_conn.close();
    ...
    {
      USRC_rollback_conn.rollback();
      eilib.DropTempTable(USRC_rollback_conn,USRC_temp_chktrans);
      eilib.DropTempTable(USRC_rollback_conn,USRC_temp_cust);
      eilib.DropTempTable(USRC_rollback_conn,USRC_temp_custlp);
      eilib.StarHubUnit_DeleteCalEvent(USRC_obj_rowvalue.USRC_tr_first,USRC_calenderIDcode,USRC_oldvalues_sdate,USRC_sh_arr[0],USRC_sh_arr[1],USRC_oldvalues_edate,USRC_sh_arr[0],USRC_sh_arr[1],'UNIT')
      eilib.StarHubUnit_CreateCalEvent(USRC_calenderIDcode,USRC_oldvalues_sdate,USRC_sh_arr[0],USRC_sh_arr[1],USRC_oldvalues_edate,USRC_sh_arr[0],USRC_sh_arr[1],'',USRC_obj_rowvalue.USRC_tr_first,'','START DATE','END DATE',USRC_obj_rowvalue.USRC_tr_five,USRC_obj_rowvalue.USRC_tr_six)
      USRC_rollback_conn.close();
