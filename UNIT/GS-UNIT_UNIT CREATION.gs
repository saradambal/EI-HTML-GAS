...
try
{
  ...
  function UCRE_getroomstamp_err(UCRE_flag)
  {
    ...
    var UCRE_errorarray =  eilib.GetErrorMessageList(UCRE_conn,'1,2,7,8,9,10,11,30,308,400,458,463,464,466,467');
  ...
  function UCRE_processForm(UCRE_unitcreation)
  {
    try{
      ...
      var UCRE_cal_conn =eilib.db_GetConnection();
      UCRE_cal_conn.setAutoCommit(false);  
      var UCRE_stmt = UCRE_cal_conn.createStatement();
      ...
      var UCRE_stmt_flag = UCRE_cal_conn.createStatement();
      var UCRE_flag_rs=UCRE_stmt_flag.executeQuery("SELECT @FLAG");
      while(UCRE_flag_rs.next())
        var UCRE_flag_save=UCRE_flag_rs.getString("@FLAG");
      if(UCRE_flag_save==1) 
      ...
      if(UCRE_flag_save==1){
        var UCRE_calenderIDcode=eilib.CUST_getCalenderId(UCRE_cal_conn);
        var  UCRE_sh_arr= eilib.getStarHubUnitCalTime(UCRE_cal_conn);
      ...
      var UCRE_getroomstamp=UCRE_getroomstamp_err(UCRE_flag_created);
      UCRE_stmt.close();
      UCRE_cal_conn.commit();
      UCRE_cal_conn.close();
      return UCRE_getroomstamp;
    }
    catch(err){
      UCRE_cal_conn.rollback();
      if(UCRE_flag_save==1){
        eilib.StarHubUnit_DeleteCalEvent(UCRE_unitnumber,UCRE_calenderIDcode,UCRE_startdate,UCRE_sh_arr[0],UCRE_sh_arr[1],UCRE_enddate,UCRE_sh_arr[0],UCRE_sh_arr[1],'UNIT')
      }
      UCRE_cal_conn.close();
