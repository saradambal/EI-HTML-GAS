...
try{
  ...
  function URSRC_getmenu_folder(basicrole){
    ...
    for(var j=0;j<URSRC_folder_id_array.length;j++){
      ...
      if(URSRC_fp_id_rs.next()){
        ...
        var URSRC_folder_name=DriveApp.getFolderById(URSRC_folder_id_array[j]).getName()
    ...
    for(var i=0;i<URSRC_folder_id_array.length;i++){
      ...
      {     
        ...
        if(URSRC_file_id!=""){       
          var URSRC_file_name=DriveApp.getFolderById(URSRC_file_id).getName()
          }     
        var URSRC_fp_id_stmt = URSRC_conn.createStatement();
        if(URSRC_file_id!=""){
  ...
  function URSRC_role_creation_save(user_rigths_form){
    ...
    if(URSRC_mainradiobutton=="ROLE CREATION"){
      ...
      var URSRC_basicrole=user_rigths_form.basicroles;
      URSRC_basicrole=URSRC_basicrole.replace("_"," ") 
      URSRC_rolecreation_stmt.execute("CALL SP_ROLE_CREATION_UPDATE('"+URSRC_customrolename+"','"+URSRC_basicrole+"','"+URSRC_menuid+"',"+fileid+",'"+UserStamp+"','"+schema_name+"',@ROLE_CREATIONUPDATE)");
      URSRC_updateSharedDocuments(URSRC_customrolename,'')      
      URSRC_rolecreation_stmt.close();  
      var URSRC_stmt_rolecreflag=URSRC_conn.createStatement();
      var URSRC_flag_rolecreselect="SELECT @ROLE_CREATIONUPDATE";
      var URSRC_flag_rolecrers=URSRC_stmt_rolecreflag.executeQuery(URSRC_flag_rolecreselect);
      while(URSRC_flag_rolecrers.next())
  ...
  function URSRC_updateSharedDocuments(URSRC_customrolename,LOGINID){
    ...
    var j=0; 
    for(var i=0; i<=URSRC_all_fileid_array.length-1;i++)
    {
      if(URSRC_fileid_array.indexOf(URSRC_all_fileid_array[i])==-1)
      {
        URSRC_diff_array[j]=URSRC_all_fileid_array[i];
        j++;
      }         
    }
    ...
    if(LOGINID==''){
      for(var j=0;j<URSRC_loginid_array.length;j++){
        for(var i=0;i<URSRC_diff_array.length;i++){
          ...
          var Folder_editor1=DriveApp.getFileById(URSRC_diff_array[i]).getEditors();
          for(var k=0;k<Folder_editor1.length;k++){
            if(URSRC_loginid_array[j]==Folder_editor1[k].getEmail().toLowerCase()){
              var remov_Folder=DriveApp.getFileById(URSRC_diff_array[i]).removeEditor(URSRC_loginid_array[j]); 
            }
          }
        }
      }
      ...
      for(var i=0;i<URSRC_diff_array.length;i++){
        ...
        for(var k=0;k<Folder_editor1.length;k++){
          if(LOGINID==Folder_editor1[k].getEmail().toLowerCase()){
            var remov_Folder=DriveApp.getFileById(URSRC_diff_array[i]).removeEditor(LOGINID); 
          }
        }
      }      
    }
    if(LOGINID==''){
      for(var k=0;k<URSRC_fileid_array.length;k++){
        var Folder_editor=DriveApp.getFileById(URSRC_fileid_array[k]).getEditors();
        var filename=DriveApp.getFileById(URSRC_fileid_array[k]).getName()
    ...
    for(var i=0; i<=URSRC_fileid_array.length-1;i++)
    {
      if(URSRC_new_fileid_array.indexOf(URSRC_fileid_array[i])==-1)
      {
        URSRC_new_diff_array[j]=URSRC_fileid_array[i];
        j++;
      }         
    }
    if(LOGINID==''){
    ...
    if(URSRC_fileid.length!=0){
      for(var i=0; i<=allid_array.length-1;i++)
      {
        ...
        {
          URSRC_new_diff_array1[K]=allid_array[i];
          K++;
        }         
      }
      if(LOGINID!=''){
        for(k=0;k< URSRC_new_diff_array1.length;k++){  
          var foldereditors=DriveApp.getFileById(URSRC_new_diff_array1[k]).getEditors()
          for(var l=0;l<foldereditors.length;l++){ 
            if(foldereditors[l].getEmail()=='')continue;
            if(LOGINID==foldereditors[l].getEmail().toLowerCase())
            var remov_Folder=DriveApp.getFileById(URSRC_new_diff_array1[k]).removeEditor(LOGINID); 
          }     
        }
      }
      else{
        ...
        for(k=0;k< URSRC_new_diff_array1.length;k++){  
          ...
          for(var l=0;l<foldereditors.length;l++){
            ...
            for(var m=0;m<URSRC_loginid_array.length;m++){
              ...
              var remov_Folder=DriveApp.getFileById(URSRC_new_diff_array1[k]).removeEditor(URSRC_loginid_array[m]); 
            }
  ...
  function URSRC_shareDocuments(URSRC_custom_role,URSRC_loginid){
    var URSRC_conn = eilib.db_GetConnection();
    ...
    for(var i=0;i<URSRC_all_fileid_array.length;i++){
      var Folder_editor1=DriveApp.getFileById(URSRC_all_fileid_array[i]).getEditors();
      for(var j=0;j<Folder_editor1.length;j++){
        if(URSRC_loginid==Folder_editor1[j].getEmail().toLowerCase())
        {          
          var remov_Folder=DriveApp.getFileById(URSRC_all_fileid_array[i]).removeEditor(URSRC_loginid);
        }        
      }    
    }
    ...
    if(URSRC_fileid.length!=0){
      ...
      for(var k=0;k< URSRC_new_diff_array.length;k++){
        ...
        for(var l=0;l<foldereditors.length;l++){ 
          if(foldereditors[l].getEmail()=='')continue;              
          if(URSRC_loginid==foldereditors[l].getEmail())
          var remov_Folder=DriveApp.getFileById(URSRC_new_diff_array[k]).removeEditor(URSRC_loginid); 
        }
      }      
    }    
  }
  ...
  function URSRC_login_creation_save(user_rigths_form){
    ...
    if(URSRC_mainradiobutton=="LOGIN CREATION"){
      ...
      URSRC_logincreation_stmt.execute("CALL SP_LOGIN_CREATION_INSERT('"+URSRC_loginid+"','"+URSRC_custom_role+"','"+URSRC_joindate+"','"+UserStamp+"',@LOGIN_CREATIONFLAG)");
      URSRC_shareDocuments(URSRC_custom_role,URSRC_loginid)      
      URSRC_AddViewer(URSRC_loginid)
      sharecalender(URSRC_loginid);
      var URSRC_stmt_lgncreflag=URSRC_conn.createStatement();
      var URSRC_flag_lgncreselect="SELECT @LOGIN_CREATIONFLAG";
      ...
      var URSRC_role_name=PropertiesService.getUserProperties().getProperty('URSRC_role_name')  
      URSRC_logincreation_stmt.execute("CALL SP_LOGIN_UPDATE('"+URSRC_loginid+"','"+URSRC_custom_role+"','"+URSRC_joindate+"','"+UserStamp+"',@LOGIN_UPDATEFLAG)")
      if(URSRC_role_name!==URSRC_custom_role){
        URSRC_updateSharedDocuments(URSRC_custom_role,URSRC_loginid) 
      }
      var URSRC_stmt_lgnupdflag=URSRC_conn.createStatement();
      var URSRC_flag_lgnupdselect="SELECT @LOGIN_UPDATEFLAG";
  ...
  function sharecalender(URSRC_loginid){
    var URSRC_conn = eilib.db_GetConnection(); 
  ...
  function URSRC_AddViewer(URSRC_loginid){  
    var URSRC_conn = eilib.db_GetConnection(); 
    var sitelink
    ...
    while(URSRC_getsite_rs.next()){
      ...
      sitelink.addViewer(URSRC_loginid);
    }
    URSRC_conn.close();    
  }  
}
